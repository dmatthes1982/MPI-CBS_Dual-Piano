...
% warning('off','all'); 
% 
% data_CF = DP_bandpass(data_CF, 1, 30, false);
% data_CU = DP_bandpass(data_CU, 1, 30, false);
% data_UF = DP_bandpass(data_UF, 1, 30, false);
% data_UU = DP_bandpass(data_UU, 1, 30, false);
% 
% warning('on','all'); 
...
data_CF_freq = DP_freqanalysis(data_CF);
data_CU_freq = DP_freqanalysis(data_CU);
data_UF_freq = DP_freqanalysis(data_UF);
data_UU_freq = DP_freqanalysis(data_UU);
...
for i=1:1:48
  data_CF.trial{i} = DP_extractSigSeg(data_CF.trial{i}, 2);
  data_CU.trial{i} = DP_extractSigSeg(data_CU.trial{i}, 2);
  data_UF.trial{i} = DP_extractSigSeg(data_UF.trial{i}, 2);
  data_UU.trial{i} = DP_extractSigSeg(data_UU.trial{i}, 2);

  data_CF.time{i} = DP_extractSigSeg(data_CF.time{i}, 2);
  data_CU.time{i} = DP_extractSigSeg(data_CU.time{i}, 2);
  data_UF.time{i} = DP_extractSigSeg(data_UF.time{i}, 2);
  data_UU.time{i} = DP_extractSigSeg(data_UU.time{i}, 2);
end
...
data_CF_psd = DP_psdanalysis(data_CF);
data_CU_psd = DP_psdanalysis(data_CU);
data_UF_psd = DP_psdanalysis(data_UF);
data_UU_psd = DP_psdanalysis(data_UU);
...
data_CF_psdseg{i} = DP_psdanalysis(data_CF_seg{i});
data_CU_psdseg{i} = DP_psdanalysis(data_CU_seg{i});
data_UF_psdseg{i} = DP_psdanalysis(data_UF_seg{i});
data_UU_psdseg{i} = DP_psdanalysis(data_UU_seg{i});
end
...
data_CF_av = DP_meanOverTrials(data_CF);
data_CU_av = DP_meanOverTrials(data_CU);
data_UF_av = DP_meanOverTrials(data_UF);
data_UU_av = DP_meanOverTrials(data_UU);
